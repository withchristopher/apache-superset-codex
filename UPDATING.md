...
A new off-by-default feature flag `DATASET_PRESENTATION_TIMEZONE` introduces an optional per-dataset presentation time zone. When enabled and a dataset opts in, the dataset's temporal columns are bucketed (grouped by time grain) and filtered in the configured IANA zone (DST-correct) rather than as stored. This includes epoch-stored timestamps (`epoch_s`/`epoch_ms`), which are decoded to their UTC instant and then expressed in the configured zone. Time-range filters shift the boundary rather than wrapping the column (for epoch columns the boundary is the corresponding epoch integer), so index/partition pruning is preserved. Relative time-range expressions ("today", "Last week") in chart queries are anchored to "now" in the configured zone, so "today" means the zone's calendar day. Explicit comparison filters (`=`, `≠`, `<`, `≤`, `>`, `≥`) on a physical temporal column interpret their value as wall-clock in the configured zone and shift the constant into storage terms, so they select the same rows as the time-range control while staying index-/partition-friendly. The feature is inert until a dataset sets a zone, and the control is only available on engines that support it (PostgreSQL and Apache Impala in the first increment). Because Impala `TIMESTAMP` is zone-less, the dataset's source time zone (configured per dataset, defaulting to UTC) is used to localize naive columns before conversion. Datasets with no zone configured behave exactly as before, so there is no change for existing deployments. Enabling the flag by default is gated on a future SIP, not on this change.

Known limitations of this first increment: multi-value (`IN`/`NOT IN`) and pattern (`LIKE`) filter operators on a temporal column are not zone-shifted; a numeric comparison-filter value on a zoned temporal column is interpreted as epoch *milliseconds* wall-clock (the frontend picker convention) — including on `epoch_s` columns, where the unzoned behaviour compared the raw number; a time-range filter that carries an explicit time grain compares against the zone-bucketed expression rather than a shifted constant (correct for timestamp columns but not sargable; for epoch columns the grain-carrying comparison inherits a pre-existing upstream type mismatch and should be avoided); annotation-layer time-range overrides remain server-anchored (chart-query relative ranges are zone-anchored); sub-day grains across a DST transition inherit the database's wall-clock resolution of the missing/duplicated hour; virtual/calculated (expression) columns are not converted; a temporal column selected as a plain (grainless) dimension via an ad-hoc expression is not converted, so it can appear as stored next to a zoned x-axis; string-stored temporal columns (a custom strftime `python_date_format`) are excluded from conversion on both the display and filter side, although relative range expressions on such a dataset still anchor at the zone's "now"; and the source time zone is one per dataset, so naive columns stored in *different* zones within one dataset cannot be represented. A `source_timezone` set without a `presentation_timezone` is inert.

Impala-specific operational notes: zone conversion runs against the *cluster's* IANA database, and Impala returns NULL (not an error) for a zone name it does not recognize — so a zone valid on the Superset server but missing on the cluster (tzdata version skew, or legacy aliases like `US/Eastern` that some distros ship in a separate `tzdata-legacy` package) produces empty charts rather than a loud failure; prefer canonical zone names and keep tzdata in sync between Superset and the cluster (for epoch columns the filter bound is computed Superset-side while the bucket is computed cluster-side, so a tzdata disagreement on a DST rule can also misalign edge buckets). Clusters started with `-use_local_tz_for_unix_timestamp_conversions` (which makes epoch decode and Kudu `UNIXTIME_MICROS` reads local-time) or `--convert_legacy_hive_parquet_utc_timestamps` (Hive-written Parquet converted UTC→local on read) change what "stored" means: epoch columns should not enable the feature there, and affected naive timestamp columns need `source_timezone` set to the cluster's zone rather than the UTC default.
