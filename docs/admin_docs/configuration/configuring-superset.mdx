...
AUTH_DB_CONFIG = {
    ...
    "password_hash_algorithm": "scrypt",  # or "pbkdf2"
...
| `password_hash_algorithm` | Hash method for new passwords (`scrypt` or `pbkdf2`; legacy `bcrypt`/`argon2` values map to `scrypt` until dedicated support lands) |
